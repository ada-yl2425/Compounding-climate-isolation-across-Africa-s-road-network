...
Usage:
    ...
    python appendix_kenya_validation.py --output sensitivity/appendix_kenya_validation.png
"""
...
def make_figure(
    ...
    ken.plot(
        ax=ax,
        column="network_efficiency_loss_pct",
        cmap=cmap,
        norm=norm,
        edgecolor="white",
        linewidth=0.3,
    ...
    ax.plot(
        ...
        linestyle="-",
        zorder=6,
        label="A3 Nairobi–Garissa corridor",
        solid_capstyle="round",
    )
    ax.plot(
        [c[0] for c in COASTAL_CORRIDOR],
        [c[1] for c in COASTAL_CORRIDOR],
        color="#1a5276",
        linewidth=1.6,
        linestyle="--",
        zorder=6,
    ...
    ax.legend(
        handles=[bdry_line, a3_line, coast_line],
        fontsize=7.2,
        loc="lower left",
        framealpha=0.88,
    )
    ...
    marker_x = Line2D(
        ...
        markersize=9,
        markeredgecolor="white",
        label="OCHA-reported road disruption",
    )
    a3_l = Line2D([0], [0], color="#154360", lw=2.0, label="A3 corridor")
    coast_l = Line2D(
        [0], [0], color="#1a5276", lw=1.6, linestyle="--", label="Garissa–Mombasa route"
    )
    ax2.legend(
        handles=[patch_ocha, patch_model, marker_x, a3_l, coast_l],
        fontsize=7.0,
        loc="lower left",
        framealpha=0.88,
    )
...
def main():
    ...
    print(
        target.sort_values("eff_loss_pctile", ascending=False)[
            [
                "district_id",
                "prov_num",
    ...
    print(
        key_pairs[["city_A", "city_B", "t_normal_h", "t_extreme_h", "increase_pct"]]
        .head(10)
        .to_string()
    )
